...
@dataclass
class ConfigServerAttackGradientOnline:
    lr: float
    num_rounds: int
    batch_size: int
    use_fixed_data: bool
...
class CLIPFederatedStrategyAttackGradientOnline(fl.server.strategy.FedAvg):
    def __init__(self, config: ConfigServerAttackGradientOnline, *args, **kwargs) -> None:
    ...
    def aggregate_evaluate(self, server_round, results, failures):
        ...
        for client, evaluate_res in results:
            ...
            if client_metrics.get("online_attack_trained", False):
                num_trained_online += 1
                online_loss_sum += client_metrics.get("online_loss", 0.0)
            ...
            for key, value in client_metrics.items():
                if isinstance(value, (int, float)):
                    ...
                    aggregated_client_metrics[key] += value
                     ...
                     aggregated_client_metrics[key] = True 
                elif isinstance(value, str):
                     if key not in aggregated_client_metrics:
                         aggregated_client_metrics[key] = value

        if num_trained_online > 0:
            aggregated_client_metrics["avg_online_attack_loss"] = online_loss_sum / num_trained_online
        aggregated_client_metrics["num_clients_trained_online"] = num_trained_online
        ...
        pred_keys_mean = [k for k in aggregated_client_metrics if k.startswith("client_prediction/") and k.endswith("_mean")]
        pred_keys_std = [k for k in aggregated_client_metrics if k.startswith("client_prediction/") and k.endswith("_std")]
        num_attackers_reporting = aggregated_client_metrics.get("attack_performed", 0) # Assumes boolean is summed to count

        if num_attackers_reporting > 0:
            for key in pred_keys_mean + pred_keys_std:
                if key in aggregated_client_metrics:
                    aggregated_client_metrics[key] /= num_attackers_reporting

        if aggregated_client_metrics and wandb.run is not None:
            try:
                client_log_data = {f"agg_client_eval/{k}": v for k, v in aggregated_client_metrics.items() if isinstance(v, (int, float, bool))} # Log numerics/bools
                wandb.log(client_log_data, step=server_round)
                logger.log(INFO, f"Round {server_round}: Logged aggregated client evaluation metrics ({len(client_log_data)} items).")
            except Exception as e:
                logger.log(WARN, f"Round {server_round}: Failed to log aggregated client metrics to WandB: {e}")
        else:
