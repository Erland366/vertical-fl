...
class CLIPFederatedStrategy(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(self, rnd, results, failures):
        ...
        for _, fit_res in results:
            client_type = fit_res.metrics["client-type"] # Mandatory available!
        ...
        labels = torch.ones(batch_size).long().to(image_embeddings.device)
        ...
        image_grads = image_embeddings.grad.detach().numpy()
        text_grads = text_embeddings.grad.detach().numpy()
        
        
        parameters_aggregated = ndarrays_to_parameters([image_grads, text_grads])
